...
import { type ComponentPropItem, type Value, UPDATE_COMPONENT_ENUM } from '~/types';

/** 统一处理子组件变化函数 */
function componentChange(event: CustomEvent): void {
  const targetDom: HTMLElement | null = (event.target as HTMLElement).closest('.configItem');
  const type: UPDATE_COMPONENT_ENUM | undefined = event.detail?.type;
  ...
  const value: Value | undefined = event.detail.value ?? undefined;

  changeCanvasComponents(type, totalKey.join('.'), value);
}
</script>
