import { type Reactive, type AsyncComponentLoader, type ShallowRef } from 'vue';
import { isNil, isObject, mapValues } from 'lodash-es';
import { type ComponentProps, type Component, type Value, isComponentPropItemConfigOptionObj } from '@/types';
...
export function reverseComponentPropsToDTO(props: ComponentProps | null | undefined): {
  ...
  return mapValues(props, item => {
    if (isObject(item) && 'value' in item) {
      let res: Value | undefined = item.value;
      if (isComponentPropItemConfigOptionObj(res)) {
        res = (res.value ?? 0) + (res.unit ?? '');
      }
      return res;
    }
    return item;
...
// 获取canvasList的Dom
export const canvasListDom: ShallowRef<HTMLElement | null> = shallowRef(null);
