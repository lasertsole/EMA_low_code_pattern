...
// props映射函数
function processComponentPropItem(item: Reactive<ComponentPropItem>): void {
  if (isEmpty(item.value)) {
    // 执行值处理函数
    if (isFunction(item.config?.valueProcessor)) {
...
export function addCanvasComponent(component: Component): void {
  ...
  if (isFunction(id.default)) {
    domProps.id = { ...id, default: id.default() };
  }

  Object.values(domProps).forEach(processComponentPropItem);
  Object.values(styleProps).forEach(processComponentPropItem);
